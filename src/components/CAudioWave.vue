...
function draw() {
  ...
  for (let i = 0; i < tempArr.length; ++i) { 
    chunks[i].style.height = Math.ceil(tempArr[tempArr.length - i - 1] * 100 / 255) + 6 + '%';
  }

  for (let i = tempArr.length; i < tempArr.length * 2; ++i) { 
    chunks[i].style.height = Math.ceil(tempArr[i - tempArr.length] * 100 / 255) + 6 + '%';
  }
